...
test_drop_reason() {
	...
	ovs_add_flow "test_drop_reason" dropreason \
		"in_port(1),eth(),eth_type(0x0800),ipv4(src=172.31.110.10,proto=17),udp(dst=6000)" 'drop(42)'
	# Drop UDP 7000 traffic with an explicit action with no error code.
	ovs_add_flow "test_drop_reason" dropreason \
		"in_port(1),eth(),eth_type(0x0800),ipv4(src=172.31.110.10,proto=17),udp(dst=7000)" 'drop(0)'

	ovs_drop_record_and_run "test_drop_reason" ip netns exec client nc -i 1 -zuv 172.31.110.20 6000
	...
	ovs_drop_record_and_run "test_drop_reason" ip netns exec client nc -i 1 -zuv 172.31.110.20 7000
